...
- **Phase**: Phase 1 complete · Phase 2 lightweight editing feature-complete (2A → 2D, 2026-05-03) · **Phase 3 first cut shipped 2026-05-07; rich AI answer card + Xiaomi MiMo provider + multi-provider default + AI review cards (Phase A) shipped 2026-05-09** — `core/ai/` provider abstraction with three first-class adapters (Anthropic / Xiaomi MiMo / OpenAI-compatible), AES-GCM-encrypted key store (Dexie v8 — `aiKeys` + `reviewBatches` + `reviewCards`), Settings → AI assistant panel with three-tab + default-provider picker, ⌘K `?` mode with Markdown-rendered streaming answers, AI-generated review cards with 24h TTL on `/__review__/:batchId`. Real-key manual smoke is the only remaining acceptance for Phase 3 v0.1. See `docs/develop/work-log.md` (latest entry) and `docs/design/ai-roadmap.md`.
- **Tests**: 892 passing. Bundle: **main 261.79 KB gz**, CSS **27.60 KB gz**, plus lazy chunks `DocumentEditSurface-*.js` **183.87 KB gz** (CodeMirror; loads on first Edit click), `CommandPalette-*.js` **9.88 KB gz** (cmdk + ⌘K + `?` ask + review action), `PaletteAskAnswer-*.js` **0.47 KB gz** (rich AI answer renderer), `ReviewPage-*.js` **3.04 KB gz** (review surface), `GenerateCardsDialog-*.js` **3.94 KB gz** (card generation modal), `SettingsPanel-*.js` **3.92 KB gz**, `ConfirmDialog-*.js` **0.56 KB gz**. Heavy renderers (Mermaid, KaTeX, Floating UI, all six M7 file renderers, command palette, AI answer surface, review surface, tag panel, TOC, shortcuts help, CodeMirror runtime, app-wide confirm dialog) live in lazy chunks — keep new heavy modules on the same pattern.
