...
- **Tests**: 924 passing. Bundle: **main 256.16 KB gz**, CSS **27.41 KB gz**, plus lazy chunks `DocumentEditSurface-*.js` **179.23 KB gz** (CodeMirror; loads on first Edit click), `CommandPalette-*.js` **9.28 KB gz** (cmdk + ⌘K + `?` ask + review action), `PaletteAskAnswer-*.js` **0.45 KB gz** (rich AI answer renderer), `ReviewPage-*.js` **2.97 KB gz** (review surface), `GenerateCardsDialog-*.js` **3.66 KB gz** (card generation modal), `SettingsPanel-*.js` **3.80 KB gz**, `ConfirmDialog-*.js` **0.56 KB gz**. Heavy renderers (Mermaid, KaTeX, Floating UI, all six M7 file renderers, command palette, AI answer surface, review surface, tag panel, TOC, shortcuts help, CodeMirror runtime, app-wide confirm dialog) live in lazy chunks — keep new heavy modules on the same pattern. **Bundle ceilings enforced by CI** via `scripts/check-bundle-size.mjs` reading `bundle-size.json`; raising a limit requires a justification in the commit.
